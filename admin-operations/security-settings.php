...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_settings'])) {
    $nn_threshold = floatval($_POST['nn_threshold']);
    $contract_address = $_POST['contract_address'];
    $contract_abi = json_decode($_POST['contract_abi'], true);

    if ($nn_threshold >= 0 && $nn_threshold <= 1 && $contract_address && $contract_abi) {
        try {
            $stmt = $pdo->prepare("UPDATE securitysettings SET nn_threshold = ?, contract_address = ?, contract_abi = ? WHERE id = 1");
            $stmt->execute([$nn_threshold, $contract_address, json_encode($contract_abi)]);
            header('Location: security-settings.php?success=' . urlencode('Security settings updated successfully.'));
            exit;
        } catch (PDOException $e) {
...
try {
    $stmt = $pdo->query("SELECT nn_threshold, contract_address, contract_abi FROM securitysettings LIMIT 1");
    $settings = $stmt->fetch(PDO::FETCH_ASSOC);
    $nn_threshold = $settings['nn_threshold'] ?: 0.7;
    $contract_address = $settings['contract_address'] ?: '';
    $contract_abi = $settings['contract_abi'] ? json_decode($settings['contract_abi'], true) : [];
} catch (PDOException $e) {
    error_log("Load security settings error: " . $e->getMessage());
    $nn_threshold = 0.7;
    $contract_address = '';
    $contract_abi = [];
}
...
<body>
    ...
    <aside class="sidebar">
        <div class="nav">
            ...
            <a href="security-settings.php" class="active"><i class="fas fa-shield-alt"></i> Security Settings</a>
        ...
        <section class="dashboard">
            <div class="dash-content">
                ...
                <form method="POST" class="settings-form">
                    <label for="nn_threshold">Neural Network Fraud Threshold (0-1):</label>
                    <input type="number" id="nn_threshold" name="nn_threshold" step="0.01" min="0" max="1" value="<?php echo htmlspecialchars($nn_threshold); ?>" required>
                    <label for="contract_address">Blockchain Contract Address:</label>
                    <input type="text" id="contract_address" name="contract_address" value="<?php echo htmlspecialchars($contract_address); ?>" required>
                    <label for="contract_abi">Contract ABI (JSON):</label>
                    <textarea id="contract_abi" name="contract_abi" required><?php echo htmlspecialchars(json_encode($contract_abi, JSON_PRETTY_PRINT)); ?></textarea>
