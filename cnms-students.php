...
if (empty($errors)) {
    ...
    try {
        ...
        foreach ($elections as &$election) {
           ...
           $query = "
                SELECT ep.position_id, ep.name AS position_name, ep.scope, ep.college_id AS position_college_id, ep.hostel_id AS position_hostel_id
                ...
                AND (
                    ...
                    OR (ep.scope = 'hostel' AND ep.hostel_id = ?)
                )
            ...
            while ($position = $result->fetch_assoc()) {
                ...
                $cand_params = [$election_id, $position_id, $association];
                $cand_types = 'iis';

                if ($position['scope'] === 'hostel' && $hostel_id) {
                    $cand_query .= " AND hostel_id = ?";
                    $cand_params[] = $hostel_id;
                    $cand_types .= 'i';
                }

                $cand_stmt = $conn->prepare($cand_query);
                if (!$cand_stmt) {
                    throw new Exception("Prepare failed: " . $conn->error);
                }
                $cand_stmt->bind_param($cand_types, ...$cand_params);
...
<head>
    ...
    <style>
        * {
        ...
        body {
            background: linear-gradient(rgba(26, 60, 52, 0.7), rgba(26, 60, 52, 0.7)), url('images/cnms.jpeg');
        ...
        .candidate-table input[type="radio"] {
            cursor: pointer;
        }
        button {
            background: #f4a261;
