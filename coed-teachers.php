...
if (isset($_SESSION['association']) && $_SESSION['association'] !== $required_association) {
    error_log("Association mismatch: expected $required_association, got " . ($_SESSION['association'] ?? 'none'));
    ...
    <style>
        * {
        ...
        .verify-section, .results-section {
            background: rgba(255, 255,  Usher, 0.1);
            backdrop-filter: blur(10px);
            padding: 20px;
            border-radius: 12px;
            box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
                        ...
                        if ($elections) {
                            foreach ($elections as $election) {
                                ...
                                while ($pos = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    ...
                                    $cand_stmt = $pdo->prepare("SELECT c.id, u.fname, u.mname, u.lname 
                                                               FROM candidates c 
