...
if (!isset($_SESSION['session_stored'])) {
    ...
    logUserActivity($conn, $user_id, 'login');
    $stmt = $conn->prepare(
...
if ($time_elapsed >= $max_session_duration) {
    error_log("Session expired due to maximum duration: $time_elapsed seconds for user_id: " . $user_id);
    session_unset();
    session_destroy();
...
if ($inactive_time >= $inactivity_timeout) {
    error_log("Session expired due to inactivity: $inactive_time seconds for user_id: " . $user_id);
    session_unset();
    session_destroy();
...
if ($association === 'UDOSO') {
    ...
    elseif ($college_id == 2) $dashboard_file = 'coed-students.php';
    elseif ($college_id == 3) $dashboard_file = 'cnms-students.php';
    else $dashboard_file = 'login.php';
} elseif ($association === 'UDOMASA') {
    if ($college_id == 1) $dashboard_file = 'cive-teachers.php';
    elseif ($college_id == 2) $dashboard_file = 'coed-teachers.php';
    elseif ($college_id == 3) $dashboard_file = 'cnms-teachers.php';
    else $dashboard_file = 'login.php';
}
