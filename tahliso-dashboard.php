...
<body>
  ...
  <section class="dashboard">
    <div class="dash-content">
      <h2>TAHLISO Election Dashboard</h2>

      <div class="overview">
        <div class="card">
          <i class="uil uil-users-alt"></i>
          <span class="text">Election Candidates</span>
          <span class="number"><?php echo $db->query("SELECT COUNT(*) FROM candidates WHERE election_type='TAHLISO'")->fetch_row()[0]; ?></span>
        </div>
        <div class="card">
        ...
        <div class="card">
          ...
          <span class="number"><?php echo $db->query("SELECT COUNT(*) FROM votes WHERE user_id='{$_SESSION['user_id']}' AND election_type='TAHLISO'")->fetch_row()[0] > 0 ? 'Voted' : 'Not Voted'; ?></span>
        </div>
      </div>
      ...
      <div class="content-section active" id="election">
        ...
        <div class="election-cards">
          <?php
          $positions = $db->query("SELECT DISTINCT position FROM candidates WHERE election_type='TAHLISO'");
          while ($pos = $positions->fetch_assoc()) {
            echo "<div class='election-card'>";
            echo "<h3>" . htmlspecialchars($pos['position']) . "</h3>";
            $candidates = $db->query("SELECT id, name FROM candidates WHERE position='" . $pos['position'] . "' AND election_type='TAHLISO'");
        ...
        <div class="vote-section">
          <table>
            ...
            while ($pos = $positions->fetch_assoc()) {
              ...
              echo "</select></td>";
              echo "<td><button onclick='submitVote(\"TAHLISO\", \"" . $pos['position'] . "\")'>Vote</button></td>";
              echo "</tr>";
            }
            ?>
          </table>
      ...
      <div class="content-section" id="verify">
        ...
        <div class="verify-section">
          ...
          <?php
          $vote = $db->query("SELECT blockchain_hash FROM votes WHERE user_id='{$_SESSION['user_id']}' AND election_type='TAHLISO' LIMIT 1")->fetch_assoc();
      ...
      <div class="content-section" id="analytics">
        ...
        <div class="analytics-section">
          <div class="chart">
            <h4>Voter Turnout</h4>
            <p><?php echo round(($db->query("SELECT COUNT(*) FROM votes WHERE election_type='TAHLISO'")->fetch_row()[0] / $db->query("SELECT COUNT(*) FROM students")->fetch_row()[0]) * 100, 1); ?>%</p>
          </div>
          <div class="chart">
            <h4>Total Votes</h4>
            <p><?php echo $db->query("SELECT COUNT(*) FROM votes WHERE election_type='TAHLISO'")->fetch_row()[0]; ?></p>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    const electionEnd = new Date('2025-07-05T23:59:59').getTime(); // Adjusted for TAHLISO
