...
try {
    ...
    header("Location: register.php?error=" . urlencode("Connection failed. Please try again later."));
    exit;
}
...
if (isset($_GET['token'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM users WHERE verification_token = ? AND is_verified = 0 AND token_expires_at > NOW()");
    $stmt->execute([$token]);
